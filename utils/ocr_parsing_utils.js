...
var occupations = JSON.parse(fs.readFileSync("/Users/stephen/Documents/nypl-directories/occupations_data/consolidated.json"));
...
function tokenize (line) {
  t1 = line.split(/(,|\.\s)/).map( (elem) => { return elem.trim(); })
  t2 = t1.filter( (elem) => { return !(/^\s*[,\.]\s*$/.test(elem)) })
  t3 = []
  t2.forEach( function(token){ t3 = t3.concat(jitter_split(token)); })
  return t3;
}
...
function category_vote(ordered_token_array) {
  ...
  ordered_token_array.forEach(function(token,i) {
    ...
    if (i == 0) {
      decisions[i]['votes'].push({'name_component' : 1.0})
      if (!TokenInterpret.contains_numbers(token)) {
        decisions[i]['votes'].push({'name_component' : 1.0})
      }
    } // End first token
    decisions[i]['votes'].push({'job_component' : job_probability_score(token, job_idx)})
    if (TokenInterpret.no_white_space(token)) {
      decisions[i]['votes'].push({'address_component' : 0.5})
    }
  }
  })
  return decisions;
...
function winner_take_all(array_most_likely_classes) {
  return array_most_likely_classes.map( (entry) => {
    modified_entry = entry;
    modified_entry['winning_class'] = Object.entries(entry['sums']).reduce( (acc,val) => { return (val[1] > acc[1]) ? val : acc})
    return modified_entry;
  })
}
        ...
        case 'predicate':
          switch (token_value) {
            case 'wid':
              ...
              if (deceased_name) {
                record['subject'].push({'value' : deceased_name, 'type' : 'deceased spouse of primary'})
              }
              break;
            case 'h':
              attach_to_next(all_decisions, index, 'address_component', [{'type' : 'home'}])
              break;
            case 'r':
              attach_to_next(all_decisions, index, 'address_component', [{'position' : 'rear'}])
              break;
          }
          break;
        case 'address_component':
          loc = {'value' : token_value}
          if (token['additional']) {
  ...
  if (current_index + 1 < decision_list.length) {
    ...
    if (next_decision['winning_class'][0] == current_token_class) {
      ...
      if (decision_list[current_index]['additional']) {
        ...
        } else {next_decision['additional'] = decision_list[current_index]['additional']}}
